...
// `CalendarEvent` now lives in the dependency-light `pimsteward-ical`
// crate (so a host-side ICS feed builder can reuse it without pulling in
// pimsteward's network/storage/credential surface). Re-export it so the
// existing `crate::forwardemail::calendar::CalendarEvent` path is unchanged.
pub use pimsteward_ical::CalendarEvent;
